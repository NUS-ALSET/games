function Simulation(config, bot1clb, bot2clb){
    ...
    this.score = [0,0];
    this.controlInfo = {keyPressed:["up", "up"]};
    this.direction = [{down:true},{down:true}];
...
Simulation.prototype.getDirections       = function(direction){
    var gamesQuant = 2;
    for(var gameId = 0; gameId < gamesQuant; gameId++){
        if(typeof direction[gameId] === 'string'){
            switch(direction[gameId].toUpperCase()){
                case 'LEFT':
                    direction[gameId] = {left:true};
                    break;
                case 'RIGHT':
                    direction[gameId] = {right:true};
                    break;
                case 'UP':
                    direction[gameId] = {up:true};
                    break;
                case 'DOWN':
                    direction[gameId] = {down:true};
                    break;
            }
        }
    }
...
Simulation.prototype.simulate = function(){
    var bot1Data = {
        player:this.bots[0], collectives:this.collectives[0], direction: this.direction[0], config: this.config, gameId: 0, controlInfo: this.controlInfo
    };
    var bot2Data = {
        player:this.bots[1], collectives:this.collectives[1], direction: this.direction[1], config: this.config, gameId: 1, controlInfo: this.controlInfo
    };
    this.direction[0]=this.bot1clb(bot1Data);
    this.direction[1]=this.bot2clb(bot2Data);
    this.direction = this.getDirections(this.direction);
    var gamesQuant = 2;
    for(var gameId = 0; gameId < gamesQuant; gameId++){
        if(this.direction[gameId]==undefined)
            this.direction[gameId] = this.directionsArr[Math.floor(Math.random()*this.directionsArr.length)];
    }
...
Simulation.prototype.moveCharacters = function(){
    var gamesQuant = 2;
    for(var gameId = 0; gameId < gamesQuant; gameId++){
        var direction = this.direction[gameId];
        var bot = this.bots[gameId];
        if(direction.up){
